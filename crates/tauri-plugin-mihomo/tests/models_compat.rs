...
#[test]
fn deserialize_smart_proxy_type_as_unknown_round_trips() -> serde_json::Result<()> {
    let proxy_type = serde_json::from_str::<ProxyType>("\"Smart\"")?;
    assert!(matches!(&proxy_type, ProxyType::Unknown(value) if value == "Smart"));
    assert_eq!(serde_json::to_string(&proxy_type)?, "\"Smart\"");
    Ok(())
}
