...
- 延续 `v1.7.7-legacy.N` 维护线，不使用另一条 `v1.7.7-legacy.rN` 分支命名
- 从 `v1.7.7` 重新构建干净 legacy 分支，保留原混乱分支作为备份和审计来源
- 高风险运行时改动未随本次发布带入，包括窗口/托盘启动、全局 SWR focus 禁用、Profile 切换重写、异步配置验证、默认端口和延迟策略调整

### Bugs Fixes

- 修复连接页面上传/下载速度排序和默认时间排序行为
- 修复代理列表选中行通过宽度和边距变化造成的布局抖动
- 修复日志页面切换后日志等级筛选丢失的问题
- 统一 Notice 信息类型和成功/错误类型的布局
- 将托盘菜单文案从 `Restart Clash` 调整为 `Restart Core`

### Build

- 重建保守的 `1.x` legacy 发布流程，默认不覆盖已有 tag/release
- 固定 legacy service 二进制来源到 `v1.7.7` portable 包
- 本地构建前检查必要 sidecar 和资源文件，缺失时提前失败
