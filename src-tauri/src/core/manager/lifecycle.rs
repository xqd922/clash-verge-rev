...
impl CoreManager {
    ...
    pub async fn change_core(&self, clash_core: &String) -> Result<()> {
        ...
        Config::verge().await.edit_draft(|d| {
            ...
            // 切换到 Smart 核心时自动启用智能转换，切回普通核心时自动关闭
            d.enable_smart_convert = Some(clash_core.as_str() == SMART_CORE_NAME);
        });
...
impl CoreManager {
    /// 将内置的 Smart 模型资源复制到应用主目录（仅 Smart 核心）
    async fn ensure_smart_model_resource(&self) {
        ...
        let src_path = match crate::utils::dirs::app_resources_dir() {
        ...
        let dest_path = match crate::utils::dirs::app_home_dir() {
            Ok(path) => path.join(SMART_MODEL_FILE),
            Err(err) => {
