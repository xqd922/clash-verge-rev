...
impl Timer {
        ...
        loop {
            tokio::select! {
                ...
                command = command_rx.recv() => {
                    match command {
                        Some(TimerCommand::Apply(new_map)) => Self::apply_timer_map(&mut queue, &mut tasks, new_map),
                        Some(TimerCommand::RunNow(uid)) => Self::run_task_now(&mut queue, &mut tasks, uid, command_tx.clone()),
                        Some(TimerCommand::TaskFinished(uid)) => Self::finish_task(&mut queue, &mut tasks, uid),
                        None => break,
                    }
                }
            }
    ...
    fn finish_task(queue: &mut DelayQueue<String>, tasks: &mut HashMap<String, TaskState>, uid: String) {
        ...
        state.running = false;
        state.key = Some(Self::schedule_task(queue, &uid, state.interval_minutes));
    }
    ...
    pub async fn get_next_update_time(&self, uid: &str) -> Option<i64> {
        ...
        (updated > 0).then_some(updated + (task_interval as i64 * 60))
    }
    ...
    async fn async_task(uid: &String) {
        ...
        match feat::update_profile(uid, None, true, false, false).await {
            Ok(()) => logging!(
                info,
                Type::Timer,
