...
/// Describe what the Service should hold for the runtime config at `config_file`.
///
/// Provider paths are rewritten to service-side destinations and recorded either as files the
/// app owns (copied by the Service) or as files the core downloads (declared as remote
/// providers so the Service can reuse its download cache).
...
/// needs to recognise — its download cache is reusable exactly while the url that produced it is
...
fn collect_provider_assets(
    ...
    for (name, provider) in providers.iter_mut() {
        ...
        // with it. Neither branch may fail the whole bundle over one odd provider: mihomo accepts
        // an `http` provider with no url, an `inline` provider carrying both a url and a path that
