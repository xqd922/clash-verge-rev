...
impl Config {
    ...
    /// 内核通过启动参数(`-ext-ctl-pipe`/`-ext-ctl-unix`)接收 IPC 监听地址。
    /// 部分内核版本在配置文件与命令行同时提供该键时会拒绝监听管道,
    /// 因此写入磁盘的运行配置一律不带它,以命令行参数为唯一来源。
    pub(crate) fn strip_ipc_listener_keys(config: &Mapping) -> Mapping {
        let mut config = config.clone();
        #[cfg(windows)]
        {
            let key = Value::from("external-controller-pipe");
            config.remove(&key);
        }
        #[cfg(unix)]
        {
            let key = Value::from("external-controller-unix");
            config.remove(&key);
        }
        config
    }
    ...
    pub async fn generate_file(typ: ConfigType) -> Result<PathBuf> {
        ...
        help::save_yaml(
            &path,
            &Self::strip_ipc_listener_keys(config),
