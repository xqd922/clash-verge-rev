...
async fn apply_builtin_scripts(
    ...
    if enable_builtin {
        let items: Vec<_> = ChainItem::builtin()
            .into_iter()
            .filter(|(s, item)| {
                s.is_support(clash_core.as_ref())
                    && (item.uid.as_str() != "verge_smart_convert" || enable_smart_convert)
            })
            .map(|(_, c)| c)
            .collect();
        for item in items {
...
#[cfg(test)]
mod tests {
    use super::{apply_builtin_scripts, cleanup_proxy_groups};
...
"#;
    ...
    #[tokio::test]
    #[allow(clippy::expect_used)]
    async fn smart_convert_builtin_is_skipped_when_disabled() {
        let config_str = r#"
profile: {}
proxy-groups:
  - name: "auto"
...
"#;
        ...
        let config = apply_builtin_scripts(config, Some("verge-mihomo-smart".into()), true, false).await;

        let group_type = config
            .get("proxy-groups")
            .and_then(|v| v.as_sequence())
        ...
        assert_eq!(group_type, Some("url-test"));
    }
}
