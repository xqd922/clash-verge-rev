...
pub async fn enhance(profiles: &IProfiles) -> Result<(Mapping, HashSet<String>, HashMap<String, ResultLog>)> {
    ...
    // 非 Smart 核心时把 smart 组还原为 url-test，保证标准内核可用
...
mod tests {
    use super::{
        ChainItem, ChainType, builtin_script_uids_for_core, cleanup_proxy_groups, ensure_lan_bind_address,
        process_global_items, process_profile_items, revert_smart_groups, use_keys,
    };
    use serde_yaml_ng::Value;
    use smartstring::alias::String;
...
"#;
    ...
    #[test]
    fn revert_smart_groups_keeps_config_on_smart_core() {
        let mut config = serde_yaml_ng::Mapping::new();
        let mut group = serde_yaml_ng::Mapping::new();
        group.insert(Value::from("name"), Value::from("auto"));
        group.insert(Value::from("type"), Value::from("smart"));
        config.insert(
            Value::from("proxy-groups"),
            Value::Sequence(vec![Value::Mapping(group)]),
        );

        let core = Some(String::from("verge-mihomo-smart"));
        let reverted = revert_smart_groups(config, &core);
        let groups = reverted.get("proxy-groups").expect("proxy-groups should exist");
        assert_eq!(groups[0].get("type").and_then(|v| v.as_str()), Some("smart"));
    }

    #[test]
    fn revert_smart_groups_converts_to_url_test_on_standard_core() {
        let mut config = serde_yaml_ng::Mapping::new();
        let mut group = serde_yaml_ng::Mapping::new();
        group.insert(Value::from("name"), Value::from("auto"));
        group.insert(Value::from("type"), Value::from("smart"));
        group.insert(Value::from("uselightgbm"), Value::from(true));
        config.insert(
            Value::from("proxy-groups"),
            Value::Sequence(vec![Value::Mapping(group)]),
        );

        let core = Some(String::from("verge-mihomo"));
        let reverted = revert_smart_groups(config, &core);
        let groups = reverted.get("proxy-groups").expect("proxy-groups should exist");
        assert_eq!(groups[0].get("type").and_then(|v| v.as_str()), Some("url-test"));
        assert!(groups[0].get("uselightgbm").is_none());
    }
}
