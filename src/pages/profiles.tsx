...
const ProfilePage = () => {
  ...
  const queuedProfileRef = useRef<string | null>(null);
  ...
  const executeBackgroundTasks = useCallback(
    (profile: string) => {
      activateSelected()
        .then(() => {
          debugLog(`[Profile] 后台处理完成: ${profile}`);
        })
        .catch((err: any) => {
  ...
  const activateProfile = useCallback(
    async function runProfileSwitch(profile: string, notifySuccess: boolean) {
      if (profiles.current === profile && !notifySuccess) {
        debugLog(`[Profile] 目标profile ${profile} 已经是当前配置，跳过切换`);
        return;
      }

      const activeSwitch = switchingProfileRef.current;
      if (activeSwitch) {
        if (activeSwitch === profile) {
      ...
      try {
        ...
        const success = await patchProfiles({ current: profile }, undefined, {
        ...
        if (success) {
          executeBackgroundTasks(profile);
        }
      } catch (err: any) {
        ...
        const nextProfile = queuedProfileRef.current;
        if (nextProfile && nextProfile !== profile) {
          queuedProfileRef.current = null;
          queueMicrotask(() => {
            void runProfileSwitch(nextProfile, true);
          });
        }
      }
    },
