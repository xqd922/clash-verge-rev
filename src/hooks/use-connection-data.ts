...
const connectionHost = (metadata: ConnectionMetadata) =>
...
const sameMetadata = (left: ConnectionMetadata, right: ConnectionMetadata) =>
  ...
  connectionHost(left) === connectionHost(right) &&
...
const flushPendingMessage = () => {
  ...
  try {
    ...
    console.error('[Connections] Failed to parse websocket payload', error)
    return
  }
...
const closeConnectionSocket = async () => {
  ...
  try {
    ...
    console.warn('Failed to close connection websocket', error)
  }
}

const scheduleReconnect = () => {
  if (!hasConnectionSubscribers() || reconnectTimer) return
...
async function connectConnectionSocket() {
  ...
  try {
    ...
    socket.addListener((message) => {
      if (connectionSocket !== socket || message.type !== 'Text') return
...
export const useConnectionData = (options?: { enabled?: boolean }) => {
  ...
  const refreshGetClashConnection = refreshConnectionData
  const clearClosedConnections = clearClosedConnectionData
...
export const useConnectionSummaryData = (options?: { enabled?: boolean }) => {
  ...
  const response = useMemo(() => ({ data }), [data])
  const refreshGetClashConnectionSummary = refreshConnectionData
