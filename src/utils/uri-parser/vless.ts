  ...
  if (proxy.network && !['tcp', 'none'].includes(proxy.network)) {
    ...
    switch (proxy.network) {
      ...
      case 'xhttp': {
        ...
        const hostValue = getIfNotBlank(host)
        const pathValue = getIfNotBlank(path)
        const modeValue = getIfNotBlank(params.mode)
        if (hostValue) xhttpOpts.host = hostValue
        if (pathValue) xhttpOpts.path = pathValue
        if (modeValue) xhttpOpts.mode = modeValue
