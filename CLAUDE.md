...
- **Branch**: Checks out `source_ref` input (default `personal`)
- **Platforms**: Windows x64/arm64, macOS x64/arm64 (unsigned), Linux x64/arm64/armv7 — same matrix as upstream `release.yml`
- **Outputs**: NSIS installers, fixed-webview2 installers (Win x64/arm64), macOS dmg, Linux deb/rpm, x64 portable zip, multi-platform update.json (regenerated by `scripts/updater.mjs`, uploaded to `updater` tag)
