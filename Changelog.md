...
- 选择性合并上游 `v2.5.2` 的稳定性改进，更新 Mihomo plugin `0.5.4`、相关类型定义与本地插件集成

### 修复

- 加固核心启停、sidecar 到 service 切换、Windows 服务协同与就绪检查，避免旧进程或过期事件覆盖当前状态
- 配置切换与 Profile 切换补齐验证、回滚和 latest-wins 队列，失败或超时后恢复已提交配置
- 修复备份、恢复、WebDAV 和本地导出的原子写入与安全校验，限制异常压缩包、路径穿越和残留脏数据
- 修复 Smart 相关模型文件准备、节点恢复、连接与日志订阅、流量统计和 URI 解析等兼容性问题
