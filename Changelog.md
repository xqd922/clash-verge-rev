...
本次为大型版本：完整合并上游 clash-verge-rev v2.5.4（358 个提交），在采纳全部后端重构与新功能的同时，保留了个人分支的 Smart 核心支持与界面定制，并针对体验做了多项对齐修复。

### 变更

**上游核心重构（全量采纳）**

- 内核管理引入全新的运行状态机（runstate）：健康检查、服务就绪探测、sidecar 与服务模式自动交接，启动与切换更稳定
- mihomo 升级改为原子化替换，失败自动清理残留内核，不再出现"升级后核心丢失"
- 配置更新全面事务化并串行化：并发修改不再互相踩踏，失败自动回滚到上一个可用状态
- 代理页数据改为后端单次 IPC 聚合下发，链式解析、固定节点等信息由后端统一计算，减少前端请求往返
- tokio 运行时线程调优（16 → 8）、批量测速通知合并、托盘菜单按需取数，降低空闲资源占用
- 定时器模块重写：移除 delay_timer 依赖，轻量模式计时更可靠
- 系统代理逻辑整体重写：按平台使用正确的 bypass 分隔符，守卫行为修正
- mihomo 插件切换为上游官方依赖（reqwest RCU 架构，支持命名管道 / Unix socket 直连），移除本地 vendor 目录；Smart 专用 API（权重排行 / 清空缓存）迁移至应用层命令，功能不变
- 前端基础设施跟随上游：React Router 8、查询层统一为 SWR 封装、运行状态改为事件驱动推送
- 新增上游能力：Linux 主题变化监听、macOS 启动守卫、静默更新器、DNS 配置初始化等

**Smart 核心支持（个人特性，完整保留）**

- LightGBM 模型随构建打包，启动时自动部署到应用目录
- 切换到 / 离开 Smart 核心时自动开启 / 关闭智能转换开关
- 订阅内 url-test/fallback/load-balance 组自动转换为 smart 类型；普通核心下自动还原为 url-test 保证兼容
- 代理组头部展示权重最高节点，节点项展示使用频率 Rank 徽标；设置页保留清空 Smart 缓存按钮

**界面（保持个人版外观）**

- 测速工具行保持在组标题下方的独立位置，展开组时出现；去除上游吸顶阴影，保持原版平面观感
- 过滤按钮恢复漏斗图标；节点徽标顺序为 来源 → 使用频率 → 类型；provider 节点不显示测速入口
- Check 文案跟随界面语言（中文显示"检测"）

**连接页**

- 表格列头点击排序：第一次点击即为降序（数值最大在最上面），再点升序，第三次取消排序

**构建 / 开发**

- 三个内核版本钉住在当前最新：稳定版 v1.19.29、Alpha alpha-2e8a04c、Smart alpha-smart-f35e876，构建产物不再随上游浮动；需要升级时改 prebuild.mjs 顶部常量即可
- 开发服务器端口调整为 Tauri 官方默认 1420 并启用 strictPort，避免与其他本地项目冲突

### 修复

- 修复混合端口在重启后不断递增的问题（上游端口管理重写）
- 修复应用退出时系统代理与服务清理不彻底的问题
- 修复 macOS 通配符守卫残留、媒体解锁检测行为异常（检测逻辑已抽离为独立模块）
- 修复内核升级失败后残留内核未清理的问题
- 修复切换页面时代理页闪烁且滚动位置丢失的问题：移除保活挂载，采用上游标准路由挂载与滚动恢复
- 修复安装器更新期间弄停系统服务后内核无法启动的问题：服务已安装但暂不可达时自动回退 Sidecar 保住连接，服务恢复后自动交接回服务模式（含 TUN）
- 修复站内更新签名校验失败（The signature was created with a different key）的问题：轮换更新签名密钥并使应用内嵌公钥与发布签名一致；本版本需手动安装一次，之后的站内更新恢复正常
- 运行配置文件不再重复携带 IPC 管道键，统一由启动参数提供，避免个别内核版本下外部控制管道监听失败
- 清理无引用的死代码与依赖（react-virtuoso、@tanstack/react-query 等），减小包体
- 合入上游 160 余项缺陷修复（连接流稳定性、延迟检测、订阅处理、WebDAV 等）
