...
- 修复刷新代理配置事件导致应用卡死的问题
- 修复 Windows 服务冷启动较慢时开机自启 TUN 被误判不可用：启动等待预算提升到 30 秒
- 修复 Windows 下应用退出后残留 sidecar 进程：sidecar 绑定 Job Object，随主程序一起退出
- 修复代理编辑页存在空 name 节点时可视化编辑崩溃
- 修复 DNS 查看器空 YAML 解析报错
- 允许局域网时自动把回环 bind-address 放宽为 *，修复局域网设备无法访问的问题
- js-yaml 导入改为 ESM 兼容写法
