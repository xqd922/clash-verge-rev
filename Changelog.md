...
- 杂项设置新增「Smart 模型训练」（位于列表末尾）：打开开关立即训练一次专属 LightGBM 模型并替换 Model.bin 自动生效（需本机 Python 环境）；可选每 3/7/14/30 天自动重新训练，关闭则不训练；训练成败以全局通知提示，过程实时显示逐行进度
...
- 关闭自动检查更新：启动不再联网检测；手动「检查更新」发现新版本时仅亮起标题栏红色 New 提示，点击后才打开更新弹窗
...
- 修复设置页 Clash 开关（如 IPv6）打开页面时先显示默认值再翻转的动画：运行时配置纳入启动预载，首帧即渲染真实状态
- 修复「Smart 模型训练」报「训练脚本资源缺失」：训练脚本此前未随安装包分发，现纳入仓库并打进打包资源
- 修复部分环境误选不可用 Python 导致训练报「缺少训练依赖」：探测改为逐一校验依赖并自动回退下一个解释器；依赖清单改为范围约束，兼容 Python 3.11–3.14
- 修复 Smart 训练在真实数据与环境下的三类失败：跳过内核写入产生的 CSV 撕裂行、pip 版 lightgbm 无 GPU 时自动回退 CPU、GBK 控制台下输出表情导致崩溃
- 修复训练过程中弹出多个控制台窗口的问题
